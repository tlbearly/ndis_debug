...
/*	  if ((this.data && this.data.name) && this.rootLayerTOC.config.title == "Hunter Reference"){
      ...
      if (!this._noCheckNode) {
		...
		if (!hiddenLayer && (this.data.parentLayerId > -1 && this.rootLayerTOC.config.radioLayers.indexOf(this.data._parentLayerInfo.name)>-1) ||
		   (this.data.parentLayerId == -1 && this.rootLayerTOC.config.radioLayers.indexOf(this.rootLayer.id)>-1)){
		    // --- tlb Add radio buttons Must use dojo so that we do not need a form tag ---
				...
				var grpName;
				if (this.data.parentLayerId > -1)
					grpName = this.rootLayerTOC.config.title.replace(/ /g,"_")+"_"+this.data._parentLayerInfo.name.replace(/ /g,"_");
				else
					grpName = this.rootLayerTOC.config.title.replace(/ /g,"_");
				
				// use html radio because dojox/mobile/RadioButton freezes scrolling
				chk = document.createElement("INPUT");
				chk.setAttribute("type","radio");
				chk.setAttribute("name",grpName);
				chk.setAttribute("id","Radio");
				chk.setAttribute("class","radioButton");
				chk.checked=this.data.visible;
				this.contentNode.childNodes[1].appendChild(chk);
				this.checkNode = chk;
		...
		// --- end tlb ---
			...
			// use html checkbox because dojox/mobile/ToggleButton freezes scrolling
			chk = document.createElement("INPUT");
			chk.setAttribute("type","checkbox");
			chk.setAttribute("class","checkBoxButton");
			chk.checked=this.data.visible;
			this.contentNode.childNodes[1].appendChild(chk);
			this.checkNode = chk;
			
			/*
			chk = new ToggleButton({
					checked: this.data.visible
			    });
			    chk.placeAt(this.checkContainerNode);
			    chk.startup();
			...
			*/
			...
			/*chk = new CheckBox({
			 ...
			 /*chk = new dojox.mobile.CheckBox({ // looks a bug in dijit. image not renderered until mouse over. bug was closed but still exist.
			...
			if (!this.rootLayerTOC.config.noLegend && this.serviceLayer && this.serviceLayer._legends && this.serviceLayer._legends.length == 1){
				...
				//domStyle.set(this.checkContainerNode, "padding","0 0 0 16px"); //use this if using arrow icons
				domStyle.set(this.checkContainerNode, "padding","0 0 0 0");	// use this if not using arrow icons
			}
    ...
    _createRootLayerNode: function(rootLayer){
      ...
      if (this.rootLayerTOC.config.slider) {
		...
		require(["dojox/mobile/Slider"],function(Slider){
			me.slider = new Slider({
				...
				style: "float:right;margin:12px;height:5px;width:100px;"
			});
    ...
    _adjustToState: function(){
      ...
      if (this.serviceLayer) {
		...
		if (outScale) {
			domStyle.set(this.labelNode, 'color', 'gray'); // tlb
			//domStyle.set(this.checkNode, 'opacity', '.3'); //tlb
		}
		else {
			domStyle.set(this.labelNode, 'color', ''); // tlb
			//domStyle.set(this.checkNode, 'opacity', '1'); //tlb
		}
