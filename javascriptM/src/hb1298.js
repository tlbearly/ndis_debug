			...
			try {
				...
				all(deferreds).then(function(results){
					...
					xmlhttp.open("POST",hb1298FileNm+"?v="+ndisVer+"&rso="+data["RSO Occurrences"]["ActivityCode"].join()+
						...
						"&cdow="+data["CDOW Occurrences"]["ActivityCode"].join(),true);
					xmlhttp.onreadystatechange = function() {
						...
						if (xmlhttp.readyState===4 && xmlhttp.status === 200) {
							...
							doc.text(marginLeft+360, y, data["CDOW Occurrences"]["ActivityCode"].length.toString());
							y += lineHt*2;
							...
							for (i=0; i<cdowArr.length; i++){
								...
								totalStr = "";
								if (!cdowArr[i]) 
									alert("Missing activity code, "+data["CDOW Occurrences"]["ActivityCode"][i]+", in hb1298_ndis.mdb.","Data Error");
								else {
							...
							require(["esri/layers/ArcGISDynamicMapServiceLayer"], function(ArcGISDynamicMapServiceLayer) {
								...
								for (i=0; i<rso.length; i++)
								{
									...
									if (!found) alert("Unable to highlight a layer. Missing '"+rso[i].LayerName+"' in Restricted Surface Occupancy map service. Should be the same name as what is used in hb1298_ndis.mdb. ","Data Error");
								}
								...
								for (i=0; i<swh.length; i++)
								{
									...
									if (!found) alert("Unable to highlight a layer. Missing '"+swh[i].LayerName+"' in Sensitive Wildlife Habitat map service. Should be the same name as what is used in hb1298_ndis.mdb. ","Data Error");								
								}
				...
				alert("Problem reading hb1298 data. ",e.message,"Code Error",e);
			}
		});	
	...
	require(["esri/tasks/ProjectParameters","esri/SpatialReference", "esri/geometry/Point","esri/graphic"],
		function(ProjectParameters,SpatialReference,Point,Graphic) {	
			...
			geometryService.project(params, function (feature) {
				...
				alert("Please enter point again. Error: "+err.message,"Warning",err);
				registry.byId("XYReportBtn").set("label", "Generate Report");
				registry.byId("XYReportBtn").set('disabled',false);
				hideLoading();
			});
...
function setPointXY(){
	require(["dijit/registry"],	function(registry) {
		try{	
			...
			// hide the menu
			closeMenu();
			document.getElementById('hb1298Pane').style.display='none';
...
function wellIDReport(){
	...
	require(["dijit/registry","esri/tasks/QueryTask","esri/tasks/query"], function(registry,QueryTask,Query) {
		...
		queryTask.execute(query, function(featureSet){  
			...
			{
				alert("Your search for the well ID, "+displayWellID+", was not successful.  Please try again. ","Warning");
				registry.byId("XYReportBtn2").set("label", "Generate Report");
				registry.byId("XYReportBtn2").set('disabled',false);
				hideLoading();
			}
...
function selectPointTool(){
	...
	require (["esri/toolbars/draw","dojo/i18n!esri/nls/jsapi","dojo/on","dojo/dom"], function (Draw,bundle,on,dom) {
		map.disableClickRecenter();
		map.hideZoomSlider();
		alert("Select a location for the report","",null,false); // No title, null error obj, no X button
		bundle.toolbars.draw.addPoint = "Select a location for the report"; // change tooltip
		toolbar2.activate(Draw.POINT);
		drawPtHandler2=on(dom.byId("mapDiv_layers"), "click", function(evt) {
			mapReport(evt);
		});
	});
}
...
function mapReport(evtObj){
	require(["esri/geometry/Point"], function(Point){
		...
		if(drawPtHandler2)drawPtHandler2.remove();
		var geometry;
		geometry = getScreenClick(evtObj);
		var pointObj =
		{
			label: document.getElementById("wellLabel2").value,
			point: new Point(geometry.x, geometry.y, map.spatialReference)
		};
		showLocation(pointObj, true);
		deselectPointTool();
	});
		...
		try{
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status === 200) {
					...
					//geometryService2.url = configData.geometryService.url;
					...
					var cp1 = new ContentPane({
						title: 'XY Location',
						content: 
							'<div>Generate Habitat Impact Report based on location.</div><br/>'+
							'<label>Well label: </label><input id="wellLabel" type="text"/><br/><br/>'+
							...
							'<div id="hb_ddDiv">'+
								'<table><tbody>'+
									...
									'<tr><td>Lat N: </td><td><input id="yDD" style="display:inline" type="text"/> &#176;</td></tr>'+
									'<tr><td>Long W:</td><td><input id="xDD" style="display:inline" type="text" onkeydown="if (event.keyCode == 13) setPointXY();"/> &#176;</td></tr>'+
								'</tbody></table>'+
							'</div>'+
							...
							'<div id="hb_dmsDiv" style="display:none">'+
								'<table><tbody>'+
									...
									'<tr style="white-space:nowrap"><td>Lat N: </td><td><input id="yDeg" style="display:inline" size="3" type="text"/>&#176;&nbsp;</td><td><input id="yMin" style="display:inline" size="3" type="text"/>&#39; </td><td><input id="ySec" style="display:inline" size="5" type="text"/> &#34;</td></tr>'+
									'<tr style="white-space:nowrap"><td>Long W:</td><td><input id="xDeg" style="display:inline" size="3" type="text"/>&#176;&nbsp;</td><td><input id="xMin" style="display:inline" size="3" type="text"/>&#39; </td><td><input id="xSec" style="display:inline" size="5" type="text" onkeydown="if (event.keyCode == 13) setPointXY();"/> &#34;</td></tr>'+
								'</tbody></table>'+
							'</div>'+
							...
							'<div id="hb_dmDiv" style="display:none">'+
								'<table><tbody>'+
									...
									'<tr style="white-space:nowrap"><td>Lat N: </td><td><input id="yDeg2" style="display:inline" size="3" type="text"/>&#176; </td><td><input id="yMin2" style="display:inline" size="5" type="text"/> &#39;</td></tr>'+
									'<tr style="white-space:nowrap"><td>Long W:</td><td><input id="xDeg2" style="display:inline" size="3" type="text"/>&#176; </td><td><input id="xMin2" style="display:inline" size="5" type="text" onkeydown="if (event.keyCode == 13) setPointXY();"/> &#39;</td></tr>'+
								'</tbody></table>'+
							'</div>'+
							'<!-- WGS84 UTM Zone 12N -->'+
					...
					var cp2 = new ContentPane({
						title: 'Well ID',
						content: 
							'<div>Generate Habitat Impact Report based on well ID.</div><br/>'+
							'<label>Well ID: </label><input id="wellID" type="text" onkeydown="if (event.keyCode == 13) wellIDReport();"/>'+
							'<p align="center">'+
					...
					var cp3 = new ContentPane({
						title: 'Select on Map',
						content: 
							'<div>Generate Habitat Impact Report based on map click.</div><br/>'+
							'<label>Well Label: </label><input id="wellLabel2" type="text"/>'+
							'<p align="center">'+
								'<button data-dojo-type="dijit/form/Button" role="presentation" type="button" title="Select well location on map." onclick="selectPointTool">Select</button>'+
