	  ...
	  //***********
		...
		function doIdentify(evt) {
			...
			if (wayPtClick) {
				...
				return; // Way Point already showing
			}
			...
			require(["dojo/dom-construct", "dojo/query", "dojo/dom", "dojo/on", "dojo/domReady!"],function(domConstruct, query, dom, on){
				...
				if ((map.infoWindow._contentPane.innerHTML.indexOf("Way Point") > -1) ||
				   (map.infoWindow._title.innerHTML != "" && map.infoWindow._title.innerHTML != "No Information")){
					if (identifyGroup != "Way Point")lastIDGroup = identifyGroup;
					identifyGroup="Way Point";
					groupContent["Way Point"]=map.infoWindow._contentPane.innerHTML;
					// Add way point to drop down list if it's not found
		...
		// Check if have cached infoTemplate content, then show it. Else do identify task then cache results in groupContent.
			require(["esri/tasks/IdentifyParameters","esri/tasks/IdentifyTask","dojo/_base/array","dojo/DeferredList","dojo/Deferred","dojo/query"], function(IdentifyParameters,IdentifyTask,array,DeferredList,Deferred,query) {
					...
					if (query("div.esriMobileInfoView",map.domNode)[0]){
						// Code for Safari
						query("div.esriMobileInfoView div",map.domNode)[0].style.WebkitTransform="translate3d(0px, 0px, 0px)";
						// Code for IE9
						query("div.esriMobileInfoView div",map.domNode)[0].style.msTransform="translate3d(0px, 0px, 0px)";
						// Standard syntax
						query("div.esriMobileInfoView div",map.domNode)[0].style.transform="translate3d(0px, 0px, 0px)";
					}
				...
				var deferreds = array.map(identifyLayerIds[identifyGroup], function (item) {
					if (item)
					{
						...
						else identifyParams.layerOption = IdentifyParameters.LAYER_OPTION_ALL;
			...
			require(["dojo/dom-construct","dojo/query","dojo/dom-class","dijit/registry"],
				function(domConstruct,query,domClass,registry){
					...
					if (identifyGroup=="Way Point") {
						...
						query("div.esriMobileNavigationItem.right1", map.domNode)[0].className="esriMobileNavigationItem right1";
						query("div.esriMobileNavigationItem.right2", map.domNode)[0].className="esriMobileNavigationItem right2";
