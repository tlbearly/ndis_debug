	  ...
	  //***********
		...
		require(["esri/tasks/IdentifyParameters","esri/symbols/SimpleLineSymbol","esri/symbols/SimpleFillSymbol","esri/symbols/PictureMarkerSymbol",
		  "dojo/_base/Color"],function(IdentifyParameters,SimpleLineSymbol,SimpleFillSymbol,PictureMarkerSymbol,Color){
							...
							for (var i=0;i<layer.length;i++) {
								...
								// Add layer without database call
									...
									if (label=="Big Game GMU Boundaries"){
										identifyLayers[identifyGroups[f]]["BighornGMUBoundaries"] = new Object();
										identifyLayers[identifyGroups[f]]["BighornGMUBoundaries"].url = settings.sheepUrl.slice(0,settings.sheepUrl.length-2);
										identifyLayers[identifyGroups[f]]["BighornGMUBoundaries"].id = settings.sheepUrl.slice(settings.sheepUrl.length-1);
										identifyLayers[identifyGroups[f]]["BighornGMUBoundaries"].geometry = "polygon";
										identifyLayers[identifyGroups[f]]["BighornGMUBoundaries"].fields = [settings.sheepField];
										identifyLayers[identifyGroups[f]]["BighornGMUBoundaries"].displaynames = ["GMU Number"];
										identifyLayers[identifyGroups[f]]["Mtn Goat GMU Boundaries"] = new Object();
										identifyLayers[identifyGroups[f]]["Mtn Goat GMU Boundaries"].url = settings.goatUrl.slice(0,settings.goatUrl.length-2);
										identifyLayers[identifyGroups[f]]["Mtn Goat GMU Boundaries"].id = settings.goatUrl.slice(settings.goatUrl.length-1);
										identifyLayers[identifyGroups[f]]["Mtn Goat GMU Boundaries"].geometry = "polygon";
										identifyLayers[identifyGroups[f]]["Mtn Goat GMU Boundaries"].fields = [settings.goatField];
										identifyLayers[identifyGroups[f]]["Mtn Goat GMU Boundaries"].displaynames = ["GMU Number"];
									}
								}
							}
						}
		...
		function doIdentify(evt) {
			...
			require(["dojo/dom-construct", "dojo/query", "dojo/dom", "dojo/on","dojo/store/Memory","dojo/domReady!"],function(domConstruct, query, dom, on,Memory){
				...
				// way point
				//if (map.infoWindow.map.infoWindow._contentPane.innerHTML.indexOf("Way Point") > -1){
				if (map.infoWindow.map.infoWindow._title.innerHTML != "" && map.infoWindow.map.infoWindow._title.innerHTML != "No Information"){
				...
				// did not click on a way point
					...
					if (identifyGroup=="Way Point") identifyGroup=identifyGroups[0];
		...
		// Check if have cached infoTemplate content, then show it. Else do identify task then cache results in groupContent.
			require(["esri/tasks/IdentifyTask","dojo/_base/array","dojo/DeferredList","dojo/_base/Deferred"], function(IdentifyTask,array,DeferredList,Deferred) {
				...
				var deferreds = array.map(identifyLayerIds[identifyGroup], function (item) {
					if (item)
					{
						task = new IdentifyTask(item.url);
						identifyParams.layerIds = item.ids;
				...
				if (identifyGroup == "GMU and Land Management"){
					if (gmu == "Bighorn GMU") {
						identifyParams.tolerance = 1;
						identifyParams.layerIds = [settings.sheepUrl.slice(settings.sheepUrl.length-1)];
						task = new IdentifyTask(settings.sheepUrl.slice(0,settings.sheepUrl.length-2));
						deferreds.push(task.execute(identifyParams,identifySuccess,handleQueryError));
					}
					else if (gmu == "Goat GMU") {
						identifyParams.tolerance = 1;
						identifyParams.layerIds = [settings.goatUrl.slice(settings.goatUrl.length-1)];
						task = new IdentifyTask(settings.goatUrl.slice(0,settings.goatUrl.length-2));
						deferreds.push(task.execute(identifyParams,identifySuccess,handleQueryError));
					}
				}
	...
	//														showIdentify();
									...
									// Layer without database call
										...
										tmpStr = r.layerName+"</strong><div style='padding-left: 10px;'>";
										if (r.layerName=="BighornGMUBoundaries") tmpStr = "Bighorn GMU Boundaries</strong><div style='padding-left: 10px;'>";
										var first = true;
										for (i=0; i<identifyLayers[identifyGroup][r.layerName].displaynames.length; i++){
			...
			require(["dojo/dom-construct","dojo/query","dojo/dom-class","dijit/registry"],
				function(domConstruct,query,domClass,registry){
					...
					if (identifyGroup=="Way Point") {
						...
						query("div.esriMobileNavigationItem.center", map.domNode)[0].style.display="block";
						domConstruct.place(idGroupCombo.domNode,query("div.esriMobileNavigationItem.center", map.domNode)[0]);
						registry.byId("idGroup").attr("displayedValue","Way Point");
						// hide these < > buttons, since we are using the graphics layer's click event instead of infoTemplate. It can only ID on the top layer.
						...
						//query("div.esriMobileNavigationItem.right1", map.domNode)[0].style.display="block";
						//query("div.esriMobileNavigationItem.right2", map.domNode)[0].style.display="block";
						...
						// display drop down, hide < > buttons
						query("div.esriMobileNavigationItem.center", map.domNode)[0].style.display="block";
						query("div.esriMobileNavigationItem.right1", map.domNode)[0].className="esriMobileNavigationItem right1 hidden";
						query("div.esriMobileNavigationItem.right2", map.domNode)[0].className="esriMobileNavigationItem right2 hidden";
						domConstruct.place(idGroupCombo.domNode,query("div.esriMobileNavigationItem.center", map.domNode)[0]);
			...
			if (numDatabaseCalls == processedDatabaseCalls) {
				...
				if (features.length == 0) {
					...
					if (identifyLayers[identifyGroup].desc) {
						str ="<div><p style='font-style:italic;top:-15px;position:relative;'>"+identifyLayers[identifyGroup].desc+"</p>No "+identifyGroup+" at this point.<br/><br/></div>";
						groupContent[identifyGroup]=str; // cache content
					}
					else{
						str = "<br/><div>No "+identifyGroup+" at this point.<br/><br/></div>"
						groupContent[identifyGroup]=str; // cache content
					}
					map.infoWindow.setContent(str);
				}
