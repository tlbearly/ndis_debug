...
function findPlaceInit() {
	...
	"dojo/query",
	"dojo/domReady!"],
	function(Search, Locator, PictureMarkerSymbol, TextSymbol, Color, Font, FeatureLayer, InfoTemplate, on, query) {
	findCombo = new Search ({
			...
			labelSymbol: new TextSymbol().setColor(new Color([0, 0, 0, 0.9])).setFont(new Font("16px", Font.STYLE_NORMAL, Font.VARIANT_NORMAL, Font.WEIGHT_BOLD, "Arial")).setOffset(15, -5).setAlign(TextSymbol.ALIGN_START),
			//autoSelect: true,// not working???
			enableButtonMode: false,  //this enables the search widget to display as a single button
			zoomScale: 144447.638572,
			allPlaceholder : "Search for a Colorado Place...",
			map: map
		}, "findCombo");
		
		//var sources = findCombo.get("sources"); // loads ESRI World geocode service
		var sources = [];
		...
		// ****** to do ****** For the featurelayers in ArcGIS in "Advanced Query Capabilities" set SupportsPagination to true to show suggestions
         sources.push({
				featureLayer: new FeatureLayer("http://ndismaps.nrel.colostate.edu/ArcGIS/rest/services/HuntingAtlas/CHA_FindAPlaceTool_Data/MapServer/1"),
				searchFields: ["COUNTYNAME"],
				displayField: "COUNTYNAME",
				exactMatch: false,
				outFields: ["COUNTYNAME"],
				name: "County",
				placeholder: "Search for a Colorado County",
				maxResults: 10,
				maxSuggestions: 10,
			...
			sources.push({
				featureLayer: new FeatureLayer("http://ndismaps.nrel.colostate.edu/ArcGIS/rest/services/HuntingAtlas/CHA_FindAPlaceTool_Data/MapServer/4"),
				searchFields: ["GMUID"],
				displayField: "GMUID",
				exactMatch: false,
				outFields: ["GMUID"],
				name: "Game Management Units",
				placeholder: "Search for a GMU ID Number",
				maxResults: 20,
				maxSuggestions: 20,
				enableSuggestions: true,
				minCharacters: 0
			...
			sources.push({
				featureLayer: new FeatureLayer("http://ndismaps.nrel.colostate.edu/ArcGIS/rest/services/HuntingAtlas/CHA_HunterBase_Map/MapServer/96"),
				searchFields: ["PropName"],
				displayField: "PropName",
				exactMatch: false,
				outFields: ["PropName"],
				name: "CPW Public Property",
				placeholder: "Search for CPW Public Property",
				maxResults: 20,
				maxSuggestions: 20,
				enableSuggestions: true,
				minCharacters: 0
			...
			sources.push({
				featureLayer: new FeatureLayer("http://ndismaps.nrel.colostate.edu/ArcGIS/rest/services/HuntingAtlas/CHA_HunterBase_Map/MapServer/99"),
				searchFields: ["MapName"],
				displayField: "MapName",
				exactMatch: false,
				outFields: ["MapName"],
				name: "Forest or Grassland",
				placeholder: "Search for a Forest/Grassland",
				maxResults: 20,
				maxSuggestions: 20,
				enableSuggestions: true,
				minCharacters: 0
			...
			sources.push({
				featureLayer: new FeatureLayer("http://ndismaps.nrel.colostate.edu/ArcGIS/rest/services/HuntingAtlas/CHA_HunterBase_Map/MapServer/97"),
				searchFields: ["NAME"],
				displayField: "NAME",
				exactMatch: false,
				outFields: ["NAME"],
				name: "Wilderness",
				placeholder: "Search for a Wilderness",
				maxResults: 20,
				maxSuggestions: 20,
				enableSuggestions: true,
				minCharacters: 0
			...
			sources.push({
				locator: new Locator(myFindService),
				singleLineFieldName: "SingleLine",
				outFields: ["address","Subregion"],
				name: "Colordo Place",
				enableSuggestions: true,
				exactMatch: false,
				maxResults: 20,
				maxSuggestions: 20,
				highlightSymbol: new PictureMarkerSymbol("https://js.arcgis.com/3.20/esri/dijit/Search/images/search-pointer.png", 36, 36).setOffset(9, 18),
        ...
        findCombo.set("sources", sources);
		findCombo.startup();
		
		// Clear Search bar when select a new source
		query(".searchMenu.sourcesMenu").on("click", function(){
			findCombo.clear();
		});
		
		// Handle Points.   Events: select-result search-clear
		findCombo.on("select-results", function(evt){
			var digits = "0123456789-";
			if (digits.indexOf(findCombo.value.substring(0, 1)) > -1 && findCombo.value.indexOf(",") > -1) {
				handleCoordinate(findCombo.value);
				return;
			}
		}); //gotoLocation);
		document.getElementById("findBtnHelp").style.display = "block";
	});
}

function findPlaceInitOld() {
...
function _clearAndZoom(point, lod) {
	try {
		require(["dijit/registry","dijit/focus"],function(registry,focusUtil){
			...
			/* new search if (!registry.byId("findCombo"))
				return;
			registry.byId("findCombo").set("store", emptyStore);
			registry.byId("findCombo").set("value", "");
...
function _displayXYPoint(point, label) {
	require(["esri/symbols/PictureMarkerSymbol", "esri/graphic", "esri/layers/GraphicsLayer"], function (PictureMarkerSymbol, Graphic, GraphicsLayer) {
		...
		label=label.replace(/''/g, "'");
		addLabel(new Graphic(point), label, searchGraphicsLayer, "11pt");
		//document.getElementById("findClear").style.opacity = 1; // for new search widget
		//document.getElementById("findClear").style.filter = "alpha(opacity=100)"; // for new search widget
	});
}
function gotoLocation(event) {
	...
	else value = event.result.name;
	if (!calledFromURL && value == "") return;
	var digits = "0123456789-";
	if (digits.indexOf(value.substring(0, 1)) > -1 && value.indexOf(",") > -1) {
		handleCoordinate(value);
		return;
	}
	if (!calledFromURL && event && event.result && event.result.name) {
		// Zoom to polygon boundary. Check if need to fill findObj with boundaries.
		if (findObj==null && ((event.result.name.toUpperCase().slice(-6, event.result.name.length) == "COUNTY") || (event.result.name.toUpperCase().slice(0, 4) == "GMU ") || (event.result.name.toUpperCase().slice(-4, event.result.name.length) == " STL") || (event.result.name.toUpperCase().indexOf(" STL ") > -1) || (event.result.name.toUpperCase().slice(-4, event.result.name.length) == " SWA") || (event.result.name.toUpperCase().indexOf(" SWA ") > -1) || (event.result.name.toUpperCase().slice(-4, event.result.name.length) == " SFU") || (event.result.name.toUpperCase().indexOf(" SFU ") > -1) || (event.result.name.toUpperCase().slice(-4, event.result.name.length) == " WWA") || (event.result.name.toUpperCase().indexOf(" WWA ") > -1) || (event.result.name.toUpperCase().slice(-15, event.result.name.length) == "NATIONAL FOREST") || (event.result.name.toUpperCase().indexOf("NATIONAL GRASSLAND") > -1) || (event.result.name.toUpperCase().indexOf("WILDERNESS") > -1)))
			loadFindBoundaries(event.result.name, event.result.feature.geometry.x, event.result.feature.geometry.y, event.result.feature.attributes.score); // load findObj
		else
			zoomToFindLocation(event.result.name, event.result.feature.geometry.x, event.result.feature.geometry.y, event.result.feature.attributes.score);
	} else {
		var found = false;
		if (!calledFromURL) {
			for (var i = 0; i < findCombo.store.data.length; i++) {
				...
				if ((value.toUpperCase() == label.toUpperCase()) || (value.toUpperCase() == findCombo.store.data[i].address.toUpperCase())) {
					...
					if (findObj==null && ((label.toUpperCase().slice(-6, label.length) == "COUNTY") || (label.toUpperCase().slice(0, 4) == "GMU ") || (label.toUpperCase().slice(-4, label.length) == " STL") || (label.toUpperCase().indexOf(" STL ") > -1) || (label.toUpperCase().slice(-4, label.length) == " SWA") || (label.toUpperCase().indexOf(" SWA ") > -1) || (label.toUpperCase().slice(-4, label.length) == " SFU") || (label.toUpperCase().indexOf(" SFU ") > -1) || (label.toUpperCase().slice(-4, label.length) == " WWA") || (label.toUpperCase().indexOf(" WWA ") > -1) || (label.toUpperCase().slice(-15, label.length) == "NATIONAL FOREST") || (label.toUpperCase().indexOf("NATIONAL GRASSLAND") > -1) || (label.toUpperCase().indexOf("WILDERNESS") > -1)))
						loadFindBoundaries(label, findCombo.store.data[i].location.x, findCombo.store.data[i].location.y, findCombo.store.data[i].attributes.score); // load findObj
					else
						zoomToFindLocation(label, findCombo.store.data[i].location.x, findCombo.store.data[i].location.y, findCombo.store.data[i].attributes.score);
					break;
				}
			}
		}
		if (!found) {
			...
			//else
				url = myFindService + "/findAddressCandidates?f=json&outFields=county,Score&Address=" + value;
			var XMLHttpRequest = createXMLhttpRequest();
			XMLHttpRequest.onreadystatechange = function () {
					...
					try {
						...
						if (!calledFromURL && dataArr.length > 1) {
							...
							if (findObj==null && ((dataArr[0].address.toUpperCase().slice(-6, dataArr[0].address.length) == "COUNTY") || (dataArr[0].address.toUpperCase().slice(0, 4) == "GMU ") || (dataArr[0].address.toUpperCase().slice(-4, dataArr[0].address.length) == " STL") || (dataArr[0].address.toUpperCase().indexOf(" STL ") > -1) || (dataArr[0].address.toUpperCase().slice(-4, dataArr[0].address.length) == " SWA") || (dataArr[0].address.toUpperCase().indexOf(" SWA ") > -1) || (dataArr[0].address.toUpperCase().slice(-4, dataArr[0].address.length) == " SFU") || (dataArr[0].address.toUpperCase().indexOf(" SFU ") > -1) || (dataArr[0].address.toUpperCase().slice(-4, dataArr[0].address.length) == " WWA") || (dataArr[0].address.toUpperCase().indexOf(" WWA ") > -1) || (dataArr[0].address.toUpperCase().slice(-15, dataArr[0].address.length) == "NATIONAL FOREST") || (dataArr[0].address.toUpperCase().indexOf("NATIONAL GRASSLAND") > -1) || (dataArr[0].address.toUpperCase().indexOf("WILDERNESS") > -1)))
								loadFindBoundaries(dataArr[0].address, dataArr[0].location.x, dataArr[0].location.y, dataArr[0].attributes.Score); // load findObj
							else
								zoomToFindLocation(dataArr[0].address, dataArr[0].location.x, dataArr[0].location.y, dataArr[0].attributes.Score);
						} else
