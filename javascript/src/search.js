...
function searchInit() {
  try {
		...
		if (xmlhttp.readyState == 4 && xmlhttp.status === 200) {
			...
			try {
				...
				require(["dojo/dom","dijit/registry","dojo/_base/lang","dojo/mouse","dojo/_base/Color","dojo/store/Memory","dijit/form/ComboBox", "dijit/form/Select", "esri/toolbars/draw",
					...
					urlUtils, Point, PictureMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol, declare, OnDemandGrid, DijitRegistry, ColumnResizer){
					function searchDrawEnd(event){
						...
						document.getElementById("clearSearchGraphics").style.display="inline-block";
					...
					function updateSearchTextStore(id){
						...
						try {
							...
							var outflds = searchObj[id].searchfield;
							...
							esriRequest({
								...
								try {
									...
									alert("Feature Search query error: "+e.message+" in search.js/updateSearchTextStore query response. response="+response,"Code Error",e);
									document.getElementById("searchLoadingImg").style.display="none";
								}	
							},function (err){
								document.getElementById("searchLoadingImg").style.display="none";
						...
						try {
							...
							// --------------------------------------------------------
								...
								if (attr == "Township Range")
								{
										...
										expr = expr.replace("[value2]", registry.byId("nsRadioForm").attr("value").nsBtn);
										expr = expr.replace("[value3]", parseInt(document.getElementById("searchRng").value));
										expr = expr.replace("[value4]", registry.byId("ewRadioForm").attr("value").ewBtn);
					...
					function queryFeaturesGraphical(geom){
						...
						require(["esri/tasks/query", "esri/tasks/QueryTask"],function(Query,QueryTask){
							...
							if (attr == "GMUs" && settings.useGMUs) {
								if (gmu == "Big Game GMU") {
									searchObj[attr].fields = settings.elkField + ",COUNTY";
									searchObj[attr].displayfields = "GMU,County";
									searchObj[attr].titlefield = settings.elkField;
									searchObj[attr].searchfield = settings.elkField;
									searchObj[attr].expression = settings.elkField + " =[value]";
									searchObj[attr].url = settings.elkUrl;
								}
								else if (gmu=="Bighorn GMU"){
							...
							if (searchObj[attr].url)
							{
								...
								queryTask.execute(query, function(featureSet) { 
									...
									alert("While using Graphical Feature Search there was a possible error in SearchWidget.xml file. Querying: "+searchObj[attr].url+" for field: "+searchObj[attr].fields +" "+error.message, "Data Error",error);    
									document.getElementById("searchMsg").style.display = "none";
									document.getElementById("searchLoadingImg").style.display="none";
											...
											for (i=0; i<recAC.length; i++)
											{
												...
												for (f=0; f<field.length; f++)
												{
													if (sort[f].toUpperCase() == "YES")
													{
														...
														else recAC[i][header[f]] = ""; // if header[f] does not exist 2/24/16
													}
												}
											}
					...
					// if select the text tab clear the draw graphics toolbar
					registry.byId("FeatureSearchTabs").watch("selectedChildWidget", function(name, oval, nval){
						if (nval.title == "Text Search") {
							searchGraphicExit();
						}
					});
					// When close the feature search widget clear the draw graphics toolbar
					dom.byId("searchDiv").addEventListener('click',function(event){
						if (!registry.byId("searchDiv").open) {
							 searchGraphicExit();
						}
					});

					// Setup button actions
					registry.byId("searchBtn").on("click",setSelection);
	...
	alert(e.message+" in javascript/search.js trying to read SearchWidget.xml file.","Code Error",e);
	document.getElementById("searchLoadingImg").style.display="none";
  }
}