...
function changePrintSize(){
	require(["esri/geometry/Point","esri/geometry/Extent","esri/SpatialReference","dojo/dom","dijit/registry"],function(Point,Extent,SpatialReference,dom,registry){
		...
		/*var ext = new Extent({
			"xmin":parseFloat(map.extent.xmin),
			"ymin":parseFloat(map.extent.ymin),
			"xmax":parseFloat(map.extent.xmax),
			"ymax":parseFloat(map.extent.ymax),
			"spatialReference": {
				"wkid":wkid
			}
		});*/
		//previewMap.setExtent(ext).then(function() {
			var sr = new SpatialReference({
				"wkid": wkid
			});
			var pt = new Point(map.extent.xmax+((map.extent.xmin-map.extent.xmax)/2),map.extent.ymin+((map.extent.ymax-map.extent.ymin)/2),sr);
			...
			previewMap.centerAndZoom(pt,map.getLevel()).then(function(){
				console.log(previewMap.getLevel());
			});
			//previewMap.centerAt(pt);
			pt = null;
			sr = null;
		//});
...
function grayOutLegend(format){
	require(["dojo/dom",],function(dom){
		document.getElementById("printMapLink").innerHTML = "";
		if (format.value != "pdf" && dom.byId("printLegend")){
  ...
  require([
      ...
      LegendLayer, GraphicsLayer, dom, registry) {
	    try{
			...
			registry.byId("print_button").set("label", "Printing...");
			...
			// pdf, gif, png8, png24, jpg, gif, geotiff, svg, svgz
			index = format.selectedIndex;
			template.format = format.options[index].value;
