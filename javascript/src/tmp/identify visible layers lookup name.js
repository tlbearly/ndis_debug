								...
								if (!found) {
									...
									if (!layer[i].getElementsByTagName("url")[0] || !layer[i].getElementsByTagName("id")[0])
										...
										identifyLayerIds[identifyGroups[f]].push({
											...
											folderLabel: folder[f].getAttribute("label"), 
											labels: new Array(layer[i].getAttribute("label"))
			...
			var deferreds = array.map(identifyLayerIds[identifyGroup], function (item) {
				if (item)
				{
					...
					if (item.id_vis_only) {
						...
						//if (item.url[item.url.length-1] == "/") url = item.url.substr(0,item.url.length-1);
						
						vis_layers = [];
						// Loop through each top layer in the TOC that is visible at this scale
						array.forEach(layers,function(layer){
							//if (layer.url == url){
							// Find the layerInfo index for the layer (in the TOC) that matches the data layer's name (layer from SettingsWidget.xml).
							var mapIds = []; // Array of data ids that match the map ids
							if (layer.layerInfos){
							for (var i=0; i < identifyParams.layerIds.length; i++){
								for (var j=0; j<layer.layerInfos.length; j++) {
									if (labels[i] == layer.layerInfos[j].name) {
										mapIds[i] = layer.layerInfos[j].id;
										break;
									}
								}
							}
							if (mapIds.length == identifyParams.layerIds.length && layer.visible == true){
									skip = false;
									var found = false;
									for (var i=0; i < identifyParams.layerIds.length; i++){
										
											// if this is the top layer and it is visible add it to list, or if this item is not visible then we are done do not add it.
											if (layer.layerInfos[mapIds[i]].parentLayerId == -1 ||
												layer.layerInfos[mapIds[i]].visible == false) {
												if (layer.layerInfos[mapIds[i]].visible == true)
											...
											else if (layer.layerInfos[layer.layerInfos[mapIds[i]].parentLayerId].parentLayerId == -1 ||
												layer.layerInfos[layer.layerInfos[mapIds[i]].parentLayerId].visible == false) {
												if (layer.layerInfos[layer.layerInfos[mapIds[i]].parentLayerId].visible == true)
													vis_layers.push(identifyParams.layerIds[i]);
											}
											// if grandparent is top layer and visible add it to list or if this item is not visible then we are done do not add it
											else if (layer.layerInfos[layer.layerInfos[layer.layerInfos[mapIds[i]].parentLayerId].parentLayerId].parentLayerId == -1 ||
												layer.layerInfos[layer.layerInfos[layer.layerInfos[mapIds[i]].parentLayerId].parentLayerId].visible == false) {
												if (layer.layerInfos[layer.layerInfos[layer.layerInfos[mapIds[i]].parentLayerId].parentLayerId].visible == true)
													vis_layers.push(identifyParams.layerIds[i]);
											}
									}
